...
eval(fs.readFileSync('./imageEncryption.js').toString());

fs.createReadStream('lena.png')
    .pipe(new PNG({
        filterType: 4
    }))
    .on('parsed', function() {	
		...
		t1 = new Date().getTime();
		this.data = imageEncryption_p(this);// or imageEncryption
		t2 = new Date().getTime();
		console.log("Execution time is " + (t2-t1)/1000 + "s");
		
        this.pack().pipe(fs.createWriteStream('encryption.png'));
    });
